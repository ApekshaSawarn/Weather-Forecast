        ...
        <>
            <Box maxW={'1400px'} m={'20px auto 5px'} p={'20px'} minH={'550px'}>
                <Grid gridTemplateColumns={['100%', 'repeat(2, 1fr)', 'repeat(2, 1fr)', '30% 27.5% 38%']} gap={'30px'}>
                    <Newbox>
                        <Box color={'#4b0082'} p={'20px'} textAlign={'center'}>
                            <Flex justify={'end'}>
                                <Icon
                        ...
                        <Grid templateColumns={'50% 50%'} h={'100%'} p={'8px'}>
                            <Box py={'10px'} pl={'15%'}>
                                {['Felt Temp.', 'Humidity', 'Wind', 'Visibility', 'Max Temp.', 'Min Temp.'].map((e, i) => (
                                    <Text key={i} color={'#9370db'} _hover={{ 'color': '#4b0082' }} fontWeight={500} mt={'15px'} fontSize={'18px'} >{e}</Text>
                                ))}
                            </Box>
                            <Box borderRadius={'30px'} bg={'#9370db'} _hover={{ 'bg': '#4b0082' }} py={'10px'} pl={'15%'}>
                ...
                <div className="tagLine">
                    <p id="madeByApeksha">Made with ❤️ by Apeksha Sawarn &nbsp;</p>
                </div>
            </Box >
        </>
    );
